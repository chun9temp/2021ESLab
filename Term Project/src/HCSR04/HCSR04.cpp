...
void HCSR04::startTimer() {
    if (!timerStarted) {
        ...
        timerStarted = true;
        echoTimeout.attach(Callback<void()>(this, &HCSR04::stopTimer), 25ms); // in case echo fall does not occur
        echo.fall(Callback<void()>(this, &HCSR04::stopTimer));
        echo.rise(NULL);
    }
}
...
void HCSR04::stopTimer() {
    ...
    if (timerStarted) {
        distance = chrono::duration<float>{timer.elapsed_time()}.count() * 1e6 / 58;
...
void HCSR04::startMeasurement() {
    trigger = 1;
    triggerTimeout.attach(Callback<void()>(this, &HCSR04::turnOffTrigger), 10us);
    echo.rise(Callback<void()>(this, &HCSR04::startTimer));
    newDataReady = false;
}
